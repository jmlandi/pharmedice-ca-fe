...
export class AuthService {
  ...
  static async registerUser(data: RegisterData): Promise<LoginResponse> {
    const response = await api.post<ApiResponse<LoginResponse>>('/auth/registrar-usuario', data);
  ...
  static async registerAdmin(data: RegisterData): Promise<LoginResponse> {
    const response = await api.post<ApiResponse<LoginResponse>>('/auth/registrar-admin', data);
  ...
  static async changePassword(data: ChangePasswordData): Promise<void> {
    const response = await api.put<ApiResponse>('/usuarios/alterar-senha', data);
  ...
  static async resendEmailVerification(): Promise<void> {
    ...
    if (!response.data.sucesso) {
      throw new Error(response.data.mensagem || 'Erro ao reenviar email de verificação');
    }
  }
}